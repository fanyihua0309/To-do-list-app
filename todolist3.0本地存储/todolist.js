...
window.onload =  function readData(){  
    ...
    if(itemList === null)   //若localStorage中不存在待办事项,itemList为null
        itemList = [];      //则将值置为空数组,否则无法使用push等数组操作方法
    localStorage.setItem("待办事项", JSON.stringify(itemList));

    itemList = JSON.parse(localStorage.getItem("待办事项")); 
 ...
 */
function handleSubmitEdit(trId, inputEditId, buttonSubmitId){    //点击提交按钮表示用户输入编辑后的内容完毕
    const tr = document.getElementById(trId);
